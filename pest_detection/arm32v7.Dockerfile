...
RUN apt-get install python3-pip libjpeg-dev libopenblas-dev libopenmpi-dev libomp-dev

RUN pip install -r ./yolov5/arm32v7.requirements.txt
RUN pip install flask==1.1.2 --index-url 'https://www.piwheels.org/simple'
...
RUN sudo -H pip3 install torch-1.7.0a0-cp37-cp37m-linux_armv7l.whl
RUN sudo -H pip3 install torchvision-0.8.0a0%2B45f960c-cp37-cp37m-linux_armv7l.whl 
